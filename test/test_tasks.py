...
from monitor import tasks, kernelci
...
class TaskTest(TestCase):
    ...
    @patch('monitor.kernelci.kernelci')
    @patch('monitor.tasks._submit_to_lava')
    @patch('monitor.tasks._notify_squadlistener')
    @patch('monitor.tasks._get_build')
    ...
    def test_monitor_boots(
        ...
        mock_kernelci.return_value = mock_reply
        mock_get_build.return_value = mock_build
        mock_submit_to_lava.return_value = "123456"
        mock_notify_squadlistener.return_value = True
        tasks.monitor_boots()
        self.assertEqual(len(SeenBuild.objects.all()), 1)
        self.assertEqual(SeenBuild.objects.all().firts().gitcommit, GIT_COMMIT)
